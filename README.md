...
- `.concept-image-grid`
  - concept 안에서 그래프/이미지 카드를 2열로 배치하는 래퍼
- `.concept-figure`
  - 이미지, 캡션, 짧은 수식 설명을 묶는 카드형 figure
- `.concept-figure-copy`
  - figure 아래에 붙는 수식/보충 설명 영역
- `.concept-figure-formula`
  - figure 아래 수식 한 줄을 들여쓰기해 보여 주는 요소
- `.article-layout`
            ...
            <div class="tistory-post-wrapper">
                ...
                <article>
                    ...
                    <div class="concept-group">
                        ...
                        <div class="concept-image-grid">
                            <figure class="concept-figure">
                                <img src="images/example.png" alt="예시 그래프">
                                <figcaption>그래프 설명</figcaption>
                                <div class="concept-figure-copy">
                                    <span class="concept-figure-formula">$f(x)$ 또는 짧은 보충 수식</span>
                                </div>
                            </figure>
                        </div>
                    </div>
...
- `Concept`는 `Section` 아래의 개념 묶음이다.
- 각 `Concept`는 반드시 `.concept-group`으로 감싼다.
- 각 `Concept`는 `h3.concept-title`을 포함한다.
- `Concept` 내부 요소는 `p.body-text`, `.math-box`, `.concept-image-grid`, `.special-block`, `.simple-list`를 사용한다.
- 같은 `Section` 안의 `Concept`는 작성 순서대로 배치한다.

## 9. Point 규칙

- `Point`는 `Concept` 아래의 세부 소주제다.
- `Point`는 `h4.point-title`을 사용한다.
- `h4.point-title` 뒤에는 `p.body-text` 또는 `.math-box`를 배치한다.
- `p.body-text`는 문단 전체에 `padding-left: 10px;`가 적용된다.
- `h4.point-title` 앞에는 작은 점 기호가 자동으로 붙는다.
...
- KaTeX 기본 display 수식 스크롤은 사용하지 않으며, 수식 박스 안에서는 스크롤바가 생기지 않도록 공용 CSS에서 display 수식 overflow를 해제한다.
- 그림과 짧은 수식 설명을 함께 배치할 때는 `.concept-image-grid > .concept-figure > figcaption + .concept-figure-copy` 구조를 사용한다.
- 그림 아래 짧은 수식은 각 줄을 `.concept-figure-formula`로 감싸 따로 들여쓴다.
...
- 각 별첨 제목은 `.special-block-title`을 사용한다.
- 요약 블록 안의 목록은 `.simple-list`를 사용해 일반 리스트와 역할을 구분한다.
